...
test('auth: POST /auth/login emite JWT e libera rotas admin', { timeout: 60_000 }, async () => {
  ...
  try {
    ...
    const lr = await fetch(`http://127.0.0.1:${port}/auth/login`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ admin_key: 'dev-admin-key' }),
    });
...
test('auth: ADMIN_AUTH_MODE=jwt_only bloqueia ADMIN_API_KEY direta', { timeout: 60_000 }, async () => {
  const port = await freePort();
  const backendRoot = path.join(path.dirname(fileURLToPath(import.meta.url)), '..');
  const proc = spawn(process.execPath, ['src/server.js'], {
    cwd: backendRoot,
  ...
  try {
    ...
    const byKey = await fetch(`http://127.0.0.1:${port}/menu/manage/items`, {
